...
class FastKNN(BaseDetector):
    ...
    def fit(self, X, y=None):
        ...
        dist_arr, _ = self.kneighbors(n_neighbors=self.n_neighbors,
    ...
    def decision_function(self, X):
        ...
        X_dist_arr, _ = self.kneighbors(X, n_neighbors=self.n_neighbors,
                                             return_distance=True)
        # print(X_dist_arr.shape)
        # initialize the output score
        pred_scores = np.zeros([X.shape[0], 1])

        distances = self._get_dist_by_method(X_dist_arr)
        # print(distances.shape)
        pred_scores = distances.reshape(1, X_dist_arr.shape[0])
        # pred_scores = distances[:, -1].reshape(-1, 1)

        # for i in range(X.shape[0]):
        #     dist_arr = X_dist_arr[i,:].reshape(1, X_dist_arr.shape[1])
        #     dist = self._get_dist_by_method(dist_arr)
        #     pred_score_i = dist[-1]
