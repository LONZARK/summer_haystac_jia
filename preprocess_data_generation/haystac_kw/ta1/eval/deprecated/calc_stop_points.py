...
if __name__ == "__main__":
    ...
    with conn.cursor() as cur:
        ...
        cur.execute("SELECT agent_id FROM agent_ids_2;")
        res = cur.fetchall()
        for i in range(len(res)):
            agent_ids2.append(res[i][0])

    print(f"agents 1: {len(agent_ids1)}")
    print(f"agents 2: {len(agent_ids2)}")

    # get rid of the old stop points tables and re-create
    with conn.cursor() as cur:
        cur.execute('DROP TABLE IF EXISTS stop_points_1;')
        conn.commit()
        cur.execute('DROP TABLE IF EXISTS stop_points_2;')
        conn.commit()
        ...
        cur.execute(ct)
        conn.commit()
        ct = ct.replace("stop_points_1", "stop_points_2")
        cur.execute(ct)
        conn.commit()

    #args = [(x, 'stop_points_1') for x in agent_ids1]
    #with Pool(10) as p:
    #    r = list(tqdm(p.imap(process_stops, iter(args)), total=len(agent_ids1)))

    args = [(x, 'stop_points_2') for x in agent_ids2]
    with Pool(10) as p:
        r = list(tqdm(p.imap(process_stops, iter(args)), total=len(agent_ids2)))
