...
from haystac_kw.config import *
from haystac_kw.third_party_ext.sumo.websocket import SimpleWebSocketServer, WebSocket, CLOSE

# import tools as sumo_tools

# TODO: fix imports such that tools --> sumo.tools
sys.path.insert(0, './third_party/sumo/tools')
# import osmGet, osmBuild, randomTrips, ptlines2flows, tileGet, sumolib
...
""")    
        ...
        opts = [sumo, "-n", self.files_relative["net"], 
                "--gui-settings-file", self.files_relative["guisettings"],
                "--duration-log.statistics",
                "--device.rerouting.adaptation-interval", "10",
                "--device.rerouting.adaptation-steps", "18",
                "-v", "--no-step-log", 
                "--save-configuration", self.files_relative["config"], 
                "--ignore-route-errors", 
                ]

        # Output simulation data 
        opts += ["--fcd-output", "fcd_data.xml",
                "--fcd-output.geo"]
        
        opts += ["--tripinfo-output", "tripinfo.xml"]
...
def get_osm(options):
    ...
    print("file://" + os.path.join(sumo_tools, "webWizard", "index.html"))
    webbrowser.open("file://" +
                    os.path.join(sumo_tools, "webWizard", "index.html"))
